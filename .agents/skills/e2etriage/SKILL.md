...
5. **Check CRD state** — compare `UnderlayList.log`, `L3VNIList.log`,
   `L2VNIList.log`, `L3VPNList.log`, `L3PassthroughList.log`, and
   `RawFRRConfigList.log` against what the test expected to configure. Check
   `RouterNodeConfigurationStatusList.log` for failed resources and degraded
   router nodes.
