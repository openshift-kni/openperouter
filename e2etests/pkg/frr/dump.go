...
func RawDump(exec executor.Executor) string {
	...
	commands := []command{
		...
		{desc: "BGP route table ipv4 vpn (SRv6)", cmd: []string{"vtysh", "-c", "show bgp ipv4 vpn"}},
		{desc: "BGP route table ipv6 vpn (SRv6)", cmd: []string{"vtysh", "-c", "show bgp ipv6 vpn"}},
		{desc: "ISIS database", cmd: []string{"vtysh", "-c", "show isis database"}},
		{desc: "ISIS interfaces", cmd: []string{"vtysh", "-c", "show isis interface"}},
		{desc: "ISIS neighbors", cmd: []string{"vtysh", "-c", "show isis neighbor"}},
