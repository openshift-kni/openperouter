...
```bash
$ oc adm must-gather --image=quay.io/redhat-user-workloads/telco-5g-tenant/openperouter-must-gather-5-1:latest
```
...
```bash
$ oc adm must-gather \
    --image=quay.io/redhat-user-workloads/telco-5g-tenant/openperouter-must-gather-5-1:latest \
...
`.tekton/openperouter-operator-must-gather-*.yaml` pipeline files and
